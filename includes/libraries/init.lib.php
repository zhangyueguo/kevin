...
class Init_Taocz_floorWidget {
    ...
    function _get_data($options = array()) {
		...
		foreach($goods_list as $k=>$v){
			$goods_list[$k]['sales'] = $v['sales'] + $v['virtual_seles'];
		}
        return $goods_list;
    }
...
class Init_Taocz_four_tabWidget {
    ...
    function _get_data($options = array(), $amount) {
		...
		$goods_list= $recom_mod->get_recommended_goods($options['img_recom_id_' . $i], 3, true, $options['img_cate_id_' . $i]);
