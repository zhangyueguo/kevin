...
var selfurl1 = "http://localhost/kevin/index.php?app=company";
...
$(function()
{
    ...
    $('img[ectype="flex"]').click(function(){
        ...
        if(status == 'open')
        {
            ...
            // props tyioocom 
			"<td><span><a href='index.php?app=company&amp;act=sect_edit&amp;id="+res[i].id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location=\"index.php?app=company&act=drop&id="+res[i].id+"\";'>"+lang.drop+"</a> | <a href='index.php?app=company&amp;act=addsector&amp;pid="+res[i].id+"'>添加字部门</a> </span></td></tr>";
			// end props
                    }
                    //将组成的字符串添加到点击对象后面
                    pr.after(str);
...
function secajax(ob)
{
    ...
    var src = $(ob).attr('src');
        if(status == 'open')
        {
            ...
            $.get(selfurl1 + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        if(res[i].add_child)
                        {
                            add_child =  " | <a href='index.php?app=company&amp;act=addsector&amp;pid="+res[i].id+"'>添加字部门</a>";
                        }
            ...
            "<td class='align_center'><span class='editable' ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].id+"' datatype='number'>"+res[i].sort_order+"</span></td>"+
			 // props tyioocom 
            "<td><span><a href='index.php?app=company&amp;act=sect_edit&amp;id="+res[i].id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location =\"index.php?app=company&amp;act=drop&amp;id="+res[i].id+"\";'>"+lang.drop+"</a>" + add_child + "</span></td></tr>";
			            // end props
                    }
                    pr.after(str);
                    change_background();
