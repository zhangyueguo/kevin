 ...
 * 
 */
function show_xiao ($msg)
{
    $a = _trigger_message(func_get_args());
    _message(serialize($a), E_XIAO);
}
...
function exception_handler($errno, $errstr, $errfile, $errline)
{
    ...
    {
        echo "<div style='font: 14px verdana'><b>Error:</b> $errstr<br/><b>Error File:</b> $errfile: [$errline]</div>";
    }
...
class Message extends MessageBase
{
    ...
    function add_link($text, $href='javascript:history.back()')
    {
        ...
        if ($this->icon == 'notice' || $this->xiao=='1' && $this->redirect == '')
        {
