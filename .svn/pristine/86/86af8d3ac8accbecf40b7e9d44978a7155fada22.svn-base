 ...
 *    @usage    none
 */
class Buyer_orderApp extends PersonalbaseApp
{
    ...
    public function Buyer_orderApp()
    {
    ...
    function recovery(){
        ...
        $model_order    =&  m('order');
        /* 只有待付款的订单可以取消 */
        $order_info     = $model_order->get("order_id={$order_id} AND buyer_id=" . $this->visitor->get('user_id') . " AND del_status " . db_create_in(array('-1'))." AND order_type = 1");
    ...
    function delete(){
        ...
        $model_order    =&  m('order');
        $order_info     = $model_order->get("order_id={$order_id} AND buyer_id=" . $this->visitor->get('user_id') . " AND status " . db_create_in(array(ORDER_CANCELED,ORDER_FINISHED))." AND order_type = 1");
...
function fengc($tid,$tname,$buyer_id,$buyer_name,$bid,$seller_name,$uere_amount,$order_sn,$leixing='推荐分成' ,$diaid='')
{
    ...
    function dustbin(){
        ...
        /* 当前用户中心菜单 */
        $this->_curitem('my_order_dustbin');
        $this->_curmenu('order_list');
    ...
    function chedidelete(){
        ...
        $model_order    =&  m('order');
        $order_info     = $model_order->get("order_id={$order_id} AND buyer_id=" . $this->visitor->get('user_id') . " AND del_status = -1 AND order_type = 1");
    ...
    function _get_orders()
    {
        ...
        if($_GET['act'] != 'dustbin'){
        	$orders = $model_order->findAll(array(
	            'conditions'    => "buyer_id=" . $this->visitor->get('user_id') . "{$conditions} AND del_status not in(-1,-2) AND order_type = 1",
	            'fields'        => 'this.*',
	            'count'         => true,
	            'limit'         => $page['limit'],
	            'order'         => 'add_time DESC',
        	...
        	$orders = $model_order->findAll(array(
	            'conditions'    => "buyer_id=" . $this->visitor->get('user_id') . " AND del_status = -1 AND order_type = 1",
	            'fields'        => 'this.*',
	            'count'         => true,
	            'limit'         => $page['limit'],
	            'order'         => 'add_time DESC',
    ...
    function _get_member_submenu()
    {
    	...
    	$dustbin = $_GET['act'] == 'dustbin' ? $menus[0]['text']='订单回收站':'';
        return $menus;
    }
