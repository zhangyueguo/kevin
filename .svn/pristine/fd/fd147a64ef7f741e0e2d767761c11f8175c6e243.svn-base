...
class RecommendModel extends BaseModel
{
    ...
    function get_recommended_goods($recom_id, $num, $default_image = true, $mall_cate_id = 0, $timeslot=array(), $sort_by='') // tyioocom
    {
        ...
        if ($recom_id == REC_NEW)
        {
            ...
            $sql = "SELECT g.goods_id,g.virtual_seles,  g.goods_name, g.default_image, gs.price, gs.stock,gs.spec_id,goodsstatistics.sales,s.store_id,s.store_name " .
...
/**
    ...
    function get_recommended_goods($recom_id, $num, $default_image = true, $mall_cate_id = 0, $timeslot=array(), $sort_by='')
    {
        ...
        if ($recom_id == REC_NEW)
        {
			...
			$sql = "SELECT g.goods_id,g.cate_id,g.virtual_seles, g.cate_name,g.goods_name,g.tags,g.brand, g.default_image, gs.price, gs.stock,goodsstatistics.sales,goodsstatistics.comments,goodsstatistics.collects,s.store_id,s.im_qq,s.im_ww,s.store_name " .
                    "FROM " . DB_PREFIX . "goods AS g " .
                    "LEFT JOIN " . DB_PREFIX . "goods_spec AS gs ON g.default_spec = gs.spec_id " .
                    "LEFT JOIN " . DB_PREFIX . "store AS s ON g.store_id = s.store_id " .
					"LEFT JOIN " . DB_PREFIX . "goods_statistics AS goodsstatistics ON goodsstatistics.goods_id=g.goods_id " .
        ...
        {
            /* 推荐商品 */
            $sql = "SELECT g.goods_id,g.cate_id,g.virtual_seles, g.cate_name,g.goods_name,g.tags,g.brand, g.default_image, gs.price, gs.stock,goodsstatistics.sales,goodsstatistics.comments,s.store_id,s.im_qq,s.im_ww,s.store_name " .
                    "FROM " . DB_PREFIX . "recommended_goods AS rg " .
                    "   LEFT JOIN " . DB_PREFIX . "goods AS g ON rg.goods_id = g.goods_id " .
                    "   LEFT JOIN " . DB_PREFIX . "goods_spec AS gs ON g.default_spec = gs.spec_id " .
                    "   LEFT JOIN " . DB_PREFIX . "store AS s ON g.store_id = s.store_id " .
