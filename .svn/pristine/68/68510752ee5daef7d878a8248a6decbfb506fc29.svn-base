...
class SearchApp extends MallbaseApp
{
    ...
    function index()
    {
        ...
        $goods_list = $goods_mod->get_list(array(
			...
			'fields'=>'g.virtual_seles,s.praise_rate,s.im_qq,s.im_ww,',// tyioocom
            'limit'      => $page['limit'],
        ));
	...
	function _get_list_goods($param,$type='recommend')
	{
		...
		if($type=='search_rec')
		{
			$order = 'virtual_seles desc,goods_id desc';
			$limit = 5;
		...
		$data = $goods_mod->find(array(
		   ...
		   "fields"     => "g.goods_id,default_image,price,goods_name,sales,virtual_seles",
		   "limit"      => $limit
		));	
    ...
    function _get_orders()
    {
        return array(
         ...
         //   'sales'        => Lang::get('sales_desc'),
		  'virtual_seles'        => Lang::get('sales_desc'),
