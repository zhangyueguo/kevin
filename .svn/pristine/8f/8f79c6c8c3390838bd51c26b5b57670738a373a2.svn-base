...
class SectModel extends BaseModel
{
    ...
    public function get_list($company_id,$parent_id = -1, $shown = false)
    {
       ...
       // $parent_id >= 0 && $conditions .= " AND parent_id = '$parent_id' AND company_id = '$company_id'";
        $parent_id >= 0 ? $conditions .= " AND parent_id = '$parent_id' AND company_id = '$company_id'" : $conditions .= " AND company_id = '$company_id'";
    ...
    {
        $cache_server =& cache_server();
        $keys = array('company_category_' . $this->id);
        foreach ($keys as $key)
        {
