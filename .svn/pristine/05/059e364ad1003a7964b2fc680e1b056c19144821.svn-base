...
/* 客户服务类型 */
define('C_S_CP', 1); // 消费者保障
define('C_S_GS', 2); // 正品保障
define('C_S_SDR', 3); // 七天退换

/* 申请客户服务状态 */
define('SERVICE_APPLYING', 0); // 申请中
define('SERVICE_AGREE',     1); // 通过
define('SERVICE_REFUSE',   2); // 拒绝
/**
 *    ECBaseApp
 *
 *    @author    Garbin
...
EOT;
    ...
    {
        $args = func_get_args();
        call_user_func_array('show_xiao', $args);
    }
...
class BaseVisitor extends Object
{
    ...
    function logout()
    {
        ...
        setcookie('wx_userid',0,time()+3600*24*100000,'/');
    }
