...
class Company_depositApp extends CompanydepositbaseApp
{
	protected $_deposit_account_mod;
	protected $_deposit_record_mod;
	protected $_deposit_recharge_mod;
	protected $_deposit_withdraw_mod;
	protected $_deposit_refund_mod;
	protected $_deposit_setting_mod;
	protected $_bank_mod;
	protected $_order_mod;
	protected $_member_mod;
	protected $_refund_mod;
	
	
	/* 构造函数 */
    function __construct()
    {
        $this->Company_depositApp();
    }

    function Company_depositApp()
    {
        parent::__construct();
    ...
    function index()
    {
		...
		$this->display('company_deposit.index.html');
    }
	...
	function recharge()
	{
		...
		if(!IS_POST)
		{
			...
			$bank_list = $this->_bank_mod->find(array('conditions'=>'user_id=0'));
			$payment_model =& m('payment');
			$payments  = $payment_model->find(array('conditions'=>"enabled=1 AND is_online=1 AND store_id=0 AND payment_code in('wxjsapi','alipay')"));			 
			/* 当前所处子菜单 */
        	$this->_curmenu('deposit_recharge');
        	/* 当前用户中心菜单 */
        	$this->_curitem('deposit');
		...
		else
		{
            ...
            /*	if (!$payment_model->in_white_list($payment_code))
				...
				if($payment_code=='wxjsapi' || $payment_code=='wxnative' )
				{
					...
					$this->display("deposit.payform_".$payment_code.".html");
					return;
			...
			else
			{
				...
				$tradesn 	= $depopay_type->submit(array(
					'trade_info' =>  array('user_id'=>$this->visitor->get('user_id'), 'party_id'=>0, 'amount'=>$money,'image'=>$image),
	...
	function wxjsapi()
	{
		$tradesn=$_GET['tradesn'];
	
		$info=$this->_deposit_recharge_mod->get("tradesn='$tradesn'");
      
	  
		$payment_model =& m('payment');
			
		$payment_info  = $payment_model->get("payment_code ='wxjsapi' AND store_id=0");
		...
		$payment    = $this->_get_platform_payment('wxjsapi', $payment_info);  
		...
		$this->_config_seo('title', Lang::get('cashier'));
		$this->assign('payform', $payment_form);
		$this->assign('money', $info['amount']);
		$this->assign('payment', $payment_info);
		header('Content-Type:text/html;charset=' . CHARSET);
		
		$this->display("deposit.payform_wxjsapi.html");
	}
...
function wxok()
	{
		...
		$tradesn=$_GET['tradesn'];	
		$info=$this->_deposit_recharge_mod->get("tradesn='$tradesn' and status='PENDING'");
		

		
		if($info){
			
			$this->_deposit_recharge_mod->edit("tradesn='$tradesn' and status='PENDING'",array('status'=>'SUCCESS'));
				$time = gmtime();				
				$deposit_account = $this->_deposit_account_mod->get(array('conditions'=>'user_id='.$info['user_id']));
				$balance   = $deposit_account['money']+$info['amount'];
			
					
					$this->_deposit_account_mod->edit($deposit_account['account_id'],array('money'=>$balance));
				 ...
				 $data_record = array(
					...
					$this->show_message('充值成功', 'back', 'index.php?app=deposit&act=rechargelist');
				
		}else{
			
			$this->show_message('充值成功', 'back', 'index.php?app=deposit&act=rechargelist');
			
/*			$this->show_message('充值失败', 'back', 'index.php?app=deposit&act=rechargelist');*/
		}
	...
	function withdraw_confirm()
	{
		...
		if($rate = $this->_deposit_setting_mod->_get_deposit_setting($this->visitor->get('user_id'),'tixian')){
		...
		{	
			...
			if($deposit_account['money'] < ($money+$fee))
		{
			$this->show_message('余额不足');
			return;
		}
    ...
    function _get_member_submenu()
    {
        $data = array(
			...
			array(
				'name'  => 'deposit_config',
				'url'	=> 'index.php?app=company_deposit&act=config',
			),
			array(
				'name'	=>	'recordlist',
				'url'	=>	'index.php?app=company_deposit&act=recordlist',
			),
			
			array(
				'name'	=>	'indraw',
				'url'	=> 	'index.php?app=company_deposit&act=drawlist',
			),
        );
		if(ACT=='withdraw' || ACT=='withdraw_confirm')
		{
			$data[] = array(
				...
				'url'	=>	'index.php?app=company_deposit&act=withdraw',
			);
		}
		if(ACT=='record')
		{
			$data[] = array(
				'name'	=>	'record_detail',
				'url'	=>	'index.php?app=company_deposit&act=record',
			);
		}
		if(ACT=='recharge')
		{
			$data[] = array(
				...
				'url'	=> 'index.php?app=company_deposit&act=recharge',
			);
		}
		if(ACT=='monthbill')
		{
			$data[] = array(
				'name'	=> 'monthbill',
				'url'	=> 'index.php?app=company_deposit&act=monthbill',
			);
		}
		if(ACT=='transfer')
		{
			$data[] = array(
				...
				'url'	=>	'index.php?app=company_deposit&act=transfer',
			);
		}
		if(ACT=='frozenlist')
		{
			$data[] = array(
				'name'	=>	'frozenlist',
				'url'	=>	'index.php?app=company_deposit&act=frozenlist',
			);
		}
