 ...
 *    @usage    none
 */
class Company_orderApp extends CompanybaseApp
{
	var $_deposit_recharge_mod;
	var $_deposit_record_mod;
	 function __construct()
    {
        $this->Company_orderApp();
    }

    function Company_orderApp()
    {
    ...
    function index()
    {
        ...
        /* 显示订单列表 */
        $this->display('company_order.index.html');
    }
    /**
     *    查看订单详情
     *
    ...
    function view()
    {
		...
		//$model_order->edit($order_id,"status=30");
    ...
    function recovery(){
        ...
        $model_order    =&  m('order');
        /* 只有待付款的订单可以取消 */
        $order_info     = $model_order->get("order_id={$order_id} AND buyer_id=" . $this->visitor->get('user_id') . " AND del_status " . db_create_in(array('-1'))." AND order_type = 2");
        $model_order->edit($order_id, array('del_status' => '0'));
        if ($model_order->has_error())
        {
            $this->pop_warning($model_order->get_error());
    ...
    function delete(){
        ...
        $model_order    =&  m('order');
        $order_info     = $model_order->get("order_id={$order_id} AND buyer_id=" . $this->visitor->get('user_id') . " AND status " . db_create_in(array(ORDER_CANCELED,ORDER_FINISHED))." AND order_type = 2");
        $model_order->edit($order_id, array('del_status' => '-1'));
        if ($model_order->has_error())
        {
            $this->pop_warning($model_order->get_error());
    ...
    function confirm_order()
    {
   	...
   	$model_order->edit($order_id, array('status' => ORDER_FINISHED, 'finished_time' => gmtime()));
				...
				$deposit_record=$this->_deposit_record_mod->get("order_sn='".$order_info['order_sn']."'"); 
				
				if($deposit_record){
				$deposit_account=$deposit_account_mod->get("user_id=".$deposit_record['party_id']);
				$money=$deposit_record['amount']+$deposit_account['money'];
				if($deposit_record['status']=='ACCEPTED'){
					if($deposit_account_mod->edit($deposit_account['account_id'],array("money"=>$money)))
					{
						$time=gmtime();
						$this->_deposit_record_mod->edit($deposit_record['record_id'],array('status'=>'SUCCESS','end_time'=>$time));
						$re_data=array(
							'tradesn'	=>	$deposit_record['tradesn'],
							'order_sn'	=>	$deposit_record['order_sn'],
							'user_id'	=>	$deposit_record['party_id'],
							'party_id'	=>	$deposit_record['user_id'],
							...
							'status'	=>	'SUCCESS',
							'payway'	=>	'预存款',
							'name'		=>	$deposit_record['name'],
							'remark'	=>	"",
							'add_time'	=>	$time,
							'pay_time'	=>	$time,
							'end_time'	=>	$time,					
						);
						$this->_deposit_record_mod->add($re_data);
					}
				}
				}
...
$setmoney = round($order_info['order_amount'] * $affiliate[$i]['pasen'], 2);	
...
function fengc($tid,$tname,$buyer_id,$buyer_name,$bid,$seller_name,$uere_amount,$order_sn,$leixing='推荐分成' ,$diaid='')
{
     ...
     */
    function dustbin(){
    	$order_info = $this->_get_orders();

    	$this->assign('dustbin','1');

        /* 当前用户中心菜单 */
        $this->_curitem('company_order_dustbin');
        $this->_curmenu('order_list');
        $this->_config_seo('title', '订单回收站 - ' . Lang::get('my_order'));
        $this->import_resource(array(
            'script' => array(
                array(
                    'path' => 'dialog/dialog.js',
    ...
    function chedidelete(){
        ...
        $model_order    =&  m('order');
        $order_info     = $model_order->get("order_id={$order_id} AND buyer_id=" . $this->visitor->get('user_id') . " AND del_status = -1 and order_type = 2");
        $model_order->edit($order_id, array('del_status' => '-2'));
        if ($model_order->has_error())
        {
            $this->pop_warning($model_order->get_error());
    ...
    function _get_orders()
    {
        ...
        if($_GET['act'] != 'dustbin'){
        	$orders = $model_order->findAll(array(
	            'conditions'    => "buyer_id=" . $this->visitor->get('user_id') . "{$conditions} AND del_status not in(-1,-2) AND order_type = 2",
	            'fields'        => 'this.*',
	            'count'         => true,
	            'limit'         => $page['limit'],
	            'order'         => 'add_time DESC',
        	...
        	$orders = $model_order->findAll(array(
	            'conditions'    => "buyer_id=" . $this->visitor->get('user_id') . " AND del_status = -1 AND order_type = 2",
	            'fields'        => 'this.*',
	            'count'         => true,
	            'limit'         => $page['limit'],
	            'order'         => 'add_time DESC',
    ...
    function _get_member_submenu()
    {
    	...
    	$dustbin = $_GET['act'] == 'dustbin' ? $menus[0]['text']='订单回收站':'订单列表';
        return $menus;
    }
